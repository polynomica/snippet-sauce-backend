...
class LangController extends Controller
{
        ...
        for ($i=0; $i < count($snip_short_form); $i++) {
            foreach ($snip_short_form[$i] as $key => $value) {
                if ($input['short_form'] == $value) {
                    return response()->json(['alert' => 'Short form already exists, Please choose a new short form.']);
                }
            }
        }
        if (in_array($input['language_name'], $data)) {
            return response()->json(['alert' => 'Language already exists.']);
        } else {
            try {
    ...
    public function update_language(Request $request, $previous_language)
    {
        ...
        if (in_array($input['language_name'], $languages)) {
            ...
            try {
                Code::where('Language', $previous_language)->update(['Language' => $input['language_name']]);
                Allot::where('Language', $previous_language)->update(['Language' => $input['language_name']]);
                ...
                Lang::where('Languages', 'exists', true)->update(['Languages' => $languages]);
                Lang::where('Languages', 'exists', true)->update(['thumbnail' => $thumbnail]);
